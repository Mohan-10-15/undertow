...
function analyzeURL(rawUrlInput) {
  ...
  if (pathSegments.length >= 5) {
    // Weight deliberately capped so "Deep path + No HTTPS" (the near-universal
    // pairing on real-world sites, incl. the 2016 legitimate corpus) stays
    // below the Medium threshold; 10+8=18 flagged ~11k legit pages in
    // validation. At 4 it only tips the scale when other signals are present.
    add(`Deep path (${pathSegments.length} segments)`, 4, 'Excessive path depth can be used to mimic a legitimate site structure.');
  }
